...
export default function Modal({ modalSize = "lg",details }: { modalSize?: "sm" | "lg",details:any }) {
  const [isOpen, setIsOpen] = useState(false);
  return (
    <div>
      <button
        onClick={() => setIsOpen(true)}
        className="rounded bg-[#1A1E23] p-2 font-medium text-[#12F7D6] transition-opacity hover:opacity-90"
      >
        View Details
      </button>
        ...
        {isOpen && (
          ...
          >
            <motion.div
              ...
              className={cn(
                "relative w-full max-w-lg cursor-default overflow-hidden rounded-xl bg-[#1A1E23]/80 backdrop-blur p-6 text-white shadow-2xl",
                {
