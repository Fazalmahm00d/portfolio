...
import Link from "next/link"; // Next.js Link component for navigation// Importing React hooks
...
const LargeDock = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-fit w-16 flex-col items-center gap-4 rounded-2xl bg-white/10 p-4 dark:bg-black/10 md:flex", // Vertical layout styles
        className,
        ...
        <AnimatePresence>
          ...
          {isHovered && (
            <motion.div
              ...
              className="absolute -left-12  w-fit -translate-x-1/2 whitespace-pre rounded-md border border-gray-200 bg-white/80 px-2 py-0.5 text-xs text-neutral-700 dark:border-neutral-900 dark:bg-neutral-800 dark:text-white"
            >
              {title} {/* Tooltip text */}
            </motion.div>
          )}
            ...
            {items.map((item, index) => (
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="flex flex-col h-10 w-10 items-center justify-center rounded-full bg-white/20 text-black shadow-md backdrop-blur-md dark:bg-black/20 dark:text-white"
                >
